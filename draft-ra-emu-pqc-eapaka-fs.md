...
- A new attribute, AT_PUB_KEM, is defined to carry the public key, which is the PQC KEM public key from the EAP server. The AT_PUB_KEM attribute will carry the ciphertext (ct) generated by the PQC KEM Encapsulation function from the EAP peer.
...
- The PQC KEM can be included first in the AT_KDF_FS attribute in the EAP-Request to indicate a higher priority for its use compared to the traditional key derivation function.
...
## Protocol Call Flow
    ...
    | yes, it will eventually respond with AT_PUB_KEM and    |     |
...
~~~
   ...
   PQ_SHARED_SECRET = kemEncaps(pKR)
   MK_PQ_SHARED_SECRET = PRF'(IK'|CK'|PQ_SHARED_SECRET|ct,"EAP-AKA' FS"| Identity)  
   K_encr = MK[0..127]
   K_aut = MK[128..383]
...
The pseudo-random function binds the shared secret to the encapsulated shared secret ciphertext (ct), achieving MAL-BIND-K-CT.  ML-KEM already is MAL-BIND-K-PK as the hash of the encapsulation key (pk) is an input to the computation of the shared secret (PQ_SHARED_SECRET). 
...
# IANA Considerations
   ...
   One new value (TBA1) in the skippable range need to be assigned by IANA 
   for AT_PUB_KEM ({{pqkem}}) in the "Attribute Types" registry 
